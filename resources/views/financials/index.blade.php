                        ...
                        <div class="row px-3">
                            <div class="col-12 col-lg-8 py-3 table-responsive">
                                <div class="d-flex justify-content-between">
                                    ...
                                    @endswitch
                                    <p class="p-0 m-0 text-capitalize py-2 mx-4">{{ $item->category }}</p>
                                    <p class="p-0 m-0 text-capitalize py-2 mx-4">Rp. {{ number_format($item->value, 0, ',', '.') }}</p>
                                </div>
                            </div>
                            <div class="col-12 col-lg-4 py-3 d-flex justify-content-center border-1 border-start">
                                {{-- <a href="{{ url('financial/'.$item->id) }}" class="btn btn-outline-light bg-gradient-faded-light border-0 shadow-none text-decoration-none fw-bold py-0 mx-auto my-auto">
                                ...
                                <a href="{{ url('financial/'.$item->id.'/edit') }}" class="btn btn-outline-light bg-gradient-faded-light border-0 shadow-none text-decoration-none fw-bold py-0 mx-auto my-auto">
                                    <div class="input-group-text bg-transparent border-0 text-secondary">
                                ...
                                <button form="delete-{{ $item->id }}" onclick="return confirm('Apakah benar Anda ingin menghapus {{ $item->name }} ?')" class="btn btn-outline-light bg-gradient-faded-light border-0 shadow-none fw-bold mx-auto py-0 my-auto">
                                    <form action="{{ url('financial/'.$item->id) }}" method="post" enctype="multipart/form-data" class="d-inline" id="delete-{{ $item->id }}">
                                        @method('delete')
                                        @csrf
                                        <div class="input-group-text bg-transparent border-0 text-secondary">
