...
<div class="d-block position-relative height-0 pb-50 pb-lg-25">
    <div class="page-breadcrumb bg-info pb-2">
        {{ view('partials.breadcumb.normal', [
            ...
            'pages_current' => 'daftar kategori penggajian']) }}
    ...
    <div class="container-fluid text-dark pt-5 mt-5">
        <div class="px-0 px-lg-4 py-4 mt-2 mt-lg-5">
                ...
                <div class="row">
                    <div class="col-12 col-lg-8">
                        <div class="d-flex justify-content-between px-2 px-lg-4">
                            ...
                            <p class="p-0 m-0 text-capitalize text-xxs">tipe</p>
                            <p class="p-0 m-0 text-capitalize text-xxs">nominal default</p>
                        </div>
                    </div>
            ...
            @can ('admin_superadmin')
                @foreach ($payroll_categories as $item)
                        ...
                        <div class="row px-3">
                            <div class="col-12 col-lg-8 py-3 table-responsive">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <p class="p-0 m-0 text-capitalize py-2 mx-4">{{ ($item->type == 'static') ? 'statis' : 'dinamis' }}</p>
                                    <p class="p-0 m-0 text-capitalize py-2 mx-4">Rp. {{ number_format($item->value_default, 0, ',', '.') }}</p>
                                </div>
                            </div>
                            <div class="col-12 col-lg-4 py-3 d-flex justify-content-center border-1 border-start">
                                {{-- <a href="{{ url('payroll_category/'.$item->id) }}" class="btn btn-outline-light bg-gradient-faded-light border-0 shadow-none text-decoration-none fw-bold py-0 mx-auto my-auto">
                                ...
                                <a href="{{ url('payroll_category/'.$item->id.'/edit') }}" class="btn btn-outline-light bg-gradient-faded-light border-0 shadow-none text-decoration-none fw-bold py-0 mx-auto my-auto">
                                    <div class="input-group-text bg-transparent border-0 text-secondary">
                                ...
                                <button form="delete-{{ $item->id }}" onclick="return confirm('Apakah benar Anda ingin menghapus {{ $item->name }} ?')" class="btn btn-outline-light bg-gradient-faded-light border-0 shadow-none fw-bold mx-auto py-0 my-auto">
                                    <form action="{{ url('payroll_category/'.$item->id) }}" method="post" enctype="multipart/form-data" class="d-inline" id="delete-{{ $item->id }}">
                                        @method('delete')
                                        @csrf
                                        <div class="input-group-text bg-transparent border-0 text-secondary">
