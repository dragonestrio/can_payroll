        ...
        <div class="px-0 px-lg-4 pb-4">
            <div class="card rounded-8-important width-100 width-lg-75 py-4 py-lg-5 px-3 px-lg-4 mx-auto">
                <form action="" method="post" enctype="multipart/form-data" id="form">
                    ...
                    @if (request()->input('date') == null)
                        <div class="py-2">
                            {{ view('forms.input-floating', ['data' => [
                                'type'          => 'month',
                                'name'          => 'date',
                                'value'         => (old('date') != null) ? old('date') : date('Y-m', strtotime($payrolls->date)),
                    ...
                    @if (request()->input('date') != null && request()->input('employee_id') == null)
                        <div class="py-2">
                            {{ view('forms.input-select-floating', ['data' => [
                                'type'          => 'select',
                                'name'          => 'employee_id',
                                'value'         => (old('employee_id') != null) ? old('employee_id') : $payrolls->employee_id,
                        ...
                        <div class="py-2">
                            {{ view('forms.input-textarea', ['data' => [
                                'type'          => 'textarea',
                                'name'          => 'description',
                                'value'         => (old('description') != null) ? old('description') : $payrolls->description,
                    ...
                    @if (request()->input('date') != null && request()->input('employee_id') != null)
                        ...
                        @foreach ($payroll_categories as $key => $value)
                            ...
                            <input type="hidden" name="payroll_details[{{ $key }}][id]" value="{{ $value->id }}">
                            ...
                            <div class="py-3 px-3 mb-3 rounded-5-important border-1 border">
                                <div class="py-2">
                                    {{ view('forms.input-floating', ['data' => [
                                        ...
                                        'value'         => (!isset($payroll_details[$key]['name']) ? $value->name : $payroll_details[$key]['name']),
                                ...
                                <div class="py-2">
                                    {{ view('forms.input-floating', ['data' => [
                                        ...
                                        'value'         => (!isset($payroll_details[$key]['value']) ? $value->value : $payroll_details[$key]['value']),
                                ...
                                <div class="py-2">
                                    {{ view('forms.input-textarea', ['data' => [
                                        ...
                                        'value'         => (!isset($payroll_details[$key]['description']) ? $value->description : $payroll_details[$key]['description']),
                ...
                <div class="d-flex justify-content-between pt-4">
                    ...
                    <button form="form" id="payroll_submit" value="1" {{ (request()->input('employee_id') == null) ? '' : 'onclick=update_payroll_details()' }}
                    class="btn btn-info py-2 px-4 rounded-5-important text-white text-uppercase">
                        <p class="p-0 m-0 text-capitalize">{{ (request()->input('employee_id') == null) ? 'selanjutnya' : 'simpan' }}</p>
                    </button>
                </div>
            </div>
        </div>
